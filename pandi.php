...
function pandi_CheckAvailability($params)
{
    ...
    $arrDomainData = $response['data'];

    // _pandiepp_log('$arrDomain', $arrDomainData);

    $results = new ResultsList();

    foreach ($arrDomainData as $domainData)
    {
        $domainSplit = explode('.', $domainData['domain']);
        $sld = array_shift($domainSplit);
        $tld = '.' . join('.', $domainSplit);

        if (!in_array($tld, $params['tlds'])) {
            continue;            
        }

        $searchResult = new SearchResult($sld, $tld);

        if ($domainData['available'] == 1) {
...
function pandi_GetDomainSuggestions($params)
{
    ...
    $searchResult = new SearchResult('danuiseng', 'id');
    $status = SearchResult::STATUS_NOT_REGISTERED;
    $searchResult->setStatus($status);
    $searchResult->setScore(30);
    $results->append($searchResult);

    $searchResult = new SearchResult('danuiseng', 'net.id');
    $status = SearchResult::STATUS_NOT_REGISTERED;
    $searchResult->setStatus($status);
    $searchResult->setScore(20);
    $results->append($searchResult);

    $searchResult = new SearchResult('danuiseng', 'ac.id');
    $status = SearchResult::STATUS_NOT_REGISTERED;
    $searchResult->setStatus($status);
    $searchResult->setScore(10);
    $results->append($searchResult);

    $searchResult = new SearchResult('danuiseng', 'biz.id');
    $status = SearchResult::STATUS_NOT_REGISTERED;
    $searchResult->setStatus($status);
    $searchResult->setScore(10);
    $results->append($searchResult);
 ...
 * the `pandi_push` function when invoked.
 *
 * @return array
 */
...
function _pandiepp_log($func, $params = false)
{

	// comment line below to see logs
	// return true;

	$handle = fopen(dirname(__FILE__) . '/log/pandiepp.log', 'a');
	ob_start();
	echo "\n================= $func =================\n";
	print_r($params);
